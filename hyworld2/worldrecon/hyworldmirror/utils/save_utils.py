...
def _build_gs_ply_data(means, scales, rotations, rgbs, opacities, quantile_threshold):
    ...
    # PLY convention for 3D Gaussian Splatting: the opacity field stores the
    # pre-sigmoid logit, and scale_* store log-scale. hyworld2 predictions hold
    # post-activation values (sigmoid-prob and exp-linear) — invert both so the
    # PLY round-trips correctly through any standard 3DGS viewer / LFS loader.
    #  (Upstream save_utils.py previously wrote probability into opacity.)
    opacities_logit = torch.logit(
        opacities[..., None].clamp(1e-6, 1.0 - 1e-6)
    )
    attributes_data = (
        means.float().detach().cpu().numpy(),
        torch.zeros_like(means).float().detach().cpu().numpy(),
        rgbs.detach().cpu().contiguous().numpy(),
        opacities_logit.detach().cpu().numpy(),
